                ...
                for exp in data[category]:
                    with st.expander(exp['title'], expanded=False):
                        for block in exp['blocks']:
                            # 【改善①】JSON保存時の一時的な2重エスケープ(\\n)を通常の改行(\n)に復元
                            content_processed = block['content'].replace("\\n", "\n")
                            
                            if block['type'] == 'text':
                                # 【改善②】マークダウンの仕様に合わせ、通常の改行を「2回改行（段落）」に自動変換して完全に改行させる
                                # これにより、編集画面で普通にエンターを押した場所がしっかり改行されます
    ...
    for e_idx, exp in enumerate(expanders):
        with st.container(border=True):
            ...
            for b_idx, block in enumerate(exp['blocks']):
                ...
                with c2:
                    # 【改善③】エラーを引き起こしていた rows=5 を廃止し、安全な height=150 (ピクセル指定) に変更
