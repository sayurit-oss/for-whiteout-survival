...
def manual_view_page():
    ...
    with tab1:
        ...
        with st.expander("2. 入会後のランクアップ（R1 ➡ R3）"):
            ...
            1. **フレンド登録**: 盟主（R5）へフレンド申請を送るよう指示する。 
            2.  **移転**: 同盟本部の周辺へ移転してもらう。  
            3.  **ランクアップ**: 上記2点が確認できたら、R1からR3へ手動で変更する。  
            4.  名前の後ろに同盟マークをつけてもらうようにうながす。
            「名前を「ʕ·ᴥ·ʔᴹᴹᶜ」に変更「ʕ·ᴥ·ʔᴹᴹᶜ」：左上アイコンタッチ、同盟ショップで、改名カードの割引があったら買ってから、おねがいします😊""")
